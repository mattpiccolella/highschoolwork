...
<div id="maincontent">
		<h3 style='padding:5px'>Upcoming Events</h3>
		<p style='padding:5px'>Below is a list of upcoming events, run by either NHS or another organizations, that you can participate. If you know of an event that is not on this list, contact one of the webmasters, officers, or advisers with your suggestion. </p>
		<?php 
		require("config.php");
		$con=mysql_connect("localhost","admin_","gator123");
		$sql = "SELECT date FROM events"; 
		...
		$derp = mysql_result($res1,$x);
		$merp = mysql_result($res2,$x);
		echo ("<b style='padding:5px'>Name of the Event: </b>" . $derp . "<br>");
		echo ("<b style='padding:5px'>Date: </b>" . $herp . "<br>");
		echo ("<b style='padding:5px'>Description: </b>" . $merp);
		echo ("<br> </br>");
		}
?>
